    ...
    for aa in a:
        ...
        for s_next in [0,1]:
            for k_next in [0,1,2,3,4]:
                ...
                if t >= T_R:
                    e_next = 0
                    x_next.append([w_next, n_next, M_next, g, e_next, s_next, z_next])
                    prob_next.append(Ps[int(s),s_next]*Pk[k_next])
                else:
                    for e_next in [0,1]:
                        x_next.append([w_next, n_next, M_next, g, e_next, s_next, z_next])
                        prob_next.append(Ps[int(s),s_next] * Pe[int(s),s_next,int(e),e_next]*Pk[k_next])
...
def dotProduct(p_next, uBTB, t):
    if t >= 45:
        return (p_next*uBTB).reshape((len(p_next)//(2*5),(2*5))).sum(axis = 1)
    else:
        return (p_next*uBTB).reshape((len(p_next)//(4*5),(4*5))).sum(axis = 1)
...
dim = (w_grid_size, n_grid_size,M_grid_size,g_grid_size,2,2,2)
dimSize = len(dim)
